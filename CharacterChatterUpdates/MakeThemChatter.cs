...
namespace Equestrian.Chatter
{
    ...
    { 
        ...
        public static bool DoBeyonder()
        {
            if (!Trainworks.Managers.PluginManager.GetAllPluginGUIDs().Contains("mod.beyonder.clan.monstertrain"))
            {
                //Ponies.Log("Failed to find Beyonder Clan.");
                return false;
            }

            ClassData clan = CustomClassManager.GetClassDataByID("Beyonder");

            if (clan == null)
            {
                Ponies.LogError("Beyonder detected but failed to find class data. Can't add battle quotes.");
                return false;
            }
            ...
            list = new List<string>()
            {
                "Idle_Chatter_LocoMotive_Key0",
                "Idle_Chatter_LocoMotive_Key1",
                "Idle_Chatter_LocoMotive_Key2"
            };
            CharacterChatterUpdater.AddIdleChatterKey(unit, list, CharacterChatterData.Gender.Neutral);
            ...
            list = new List<string>()
            {
                "Idle_Chatter_Epidemial_Key0",
                "Idle_Chatter_Epidemial_Key1",
                "Idle_Chatter_Epidemial_Key2"
            };
            CharacterChatterUpdater.AddIdleChatterKey(unit, list, CharacterChatterData.Gender.Neutral);
