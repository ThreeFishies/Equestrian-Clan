...
public sealed class CustomCardEffectAddTempCardUpgradeToUnitsByTrigger : CardEffectBase, ICardEffectTipTooltip
{
	// Token: 0x060006BB RID: 1723 RVA: 0x00021094 File Offset: 0x0001F294
	public override bool TestEffect(CardEffectState cardEffectState, CardEffectParams cardEffectParams)
	{
		CharacterTriggerData.Trigger allowedTrigger = cardEffectState.GetParamTrigger();

		bool hasAllowedTarget = false;

		foreach (CharacterState target in cardEffectParams.targets)
		{
			List<CharacterTriggerState>.Enumerator enumerator1 = target.GetTriggers().GetEnumerator();
			while (enumerator1.MoveNext())
			{
				if (enumerator1.Current.GetTrigger() == allowedTrigger)
				{
